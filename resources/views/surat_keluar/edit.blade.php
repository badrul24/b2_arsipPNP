...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Form Edit Data Surat Keluar</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="max-w-4xl">
            <form action="{{ route('surat_keluar.update', $suratKeluar->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nomor_surat_keluar" id="nomor_surat_keluar"
                                ...
                                value="{{ old('nomor_surat_keluar', $suratKeluar->nomor_surat_keluar) }}" required>
                        ...
                        <div>
                            ...
                            <select name="penerima" id="penerima"
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->name }}" {{ old('penerima', $suratKeluar->penerima) == $user->name ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="jenis_surat" id="jenis_surat"
                                ...
                                <option value="Surat Undangan" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Undangan' ? 'selected' : '' }}>Surat Undangan</option>
                                <option value="Surat Pemberitahuan" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Pemberitahuan' ? 'selected' : '' }}>Surat Pemberitahuan</option>
                                <option value="Surat Permohonan" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Permohonan' ? 'selected' : '' }}>Surat Permohonan</option>
                                <option value="Surat Keputusan" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Keputusan' ? 'selected' : '' }}>Surat Keputusan</option>
                                <option value="Surat Edaran" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Edaran' ? 'selected' : '' }}>Surat Edaran</option>
                                <option value="Surat Tugas" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Tugas' ? 'selected' : '' }}>Surat Tugas</option>
                                <option value="Surat Pengantar" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Pengantar' ? 'selected' : '' }}>Surat Pengantar</option>
                                <option value="Surat Keterangan" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Keterangan' ? 'selected' : '' }}>Surat Keterangan</option>
                                <option value="Surat Lainnya" {{ old('jenis_surat', $suratKeluar->jenis_surat) == 'Surat Lainnya' ? 'selected' : '' }}>Surat Lainnya</option>
                            </select>
                            @error('jenis_surat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="sifat_surat" id="sifat_surat"
                                ...
                                <option value="Sangat Penting" {{ old('sifat_surat', $suratKeluar->sifat_surat) == 'Sangat Penting' ? 'selected' : '' }}>Sangat Penting</option>
                            ...
                            <select name="jurusan_id" id="jurusan_id" {{ $isDisabled }}
                                ...
                                @foreach($jurusans as $jurusan)
                                    <option value="{{ $jurusan->id }}"
                                        {{ old('jurusan_id', $suratKeluar->jurusan_id) == $jurusan->id ? 'selected' : '' }}>
                                        {{ $jurusan->nama_jurusan }}
                                    </option>
                                @endforeach
                            </select>
                            @if($isDisabled)
                                <input type="hidden" name="jurusan_id" value="{{ old('jurusan_id', $suratKeluar->jurusan_id) }}">
                            @endif
                            @error('jurusan_id')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                            ...
                            <select name="divisi_id" id="divisi_id" {{ $isDivisiDisabled }}
                                ...
                                @foreach($divisis as $divisi)
                                    <option value="{{ $divisi->id }}"
                                        {{ old('divisi_id', $suratKeluar->divisi_id) == $divisi->id ? 'selected' : '' }}>
                                        {{ $divisi->nama_divisi }}
                                    </option>
                                @endforeach
                            </select>
                            @if($isDivisiDisabled)
                                <input type="hidden" name="divisi_id" value="{{ old('divisi_id', $suratKeluar->divisi_id) }}">
                            @endif
                            @error('divisi_id')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            @if($suratKeluar->file_surat_path)
                                <p class="text-sm text-gray-600 mb-1">File saat ini:
                                    <a href="{{ route('surat_keluar.download', $suratKeluar->id) }}" target="_blank" class="text-blue-600 underline">
                                        {{ $suratKeluar->nama_file_surat_asli }}
                                    </a>
                                </p>
                            @endif
                            ...
                            <p class="text-xs text-gray-500 mt-1">Kosongkan jika tidak ingin mengubah file. PDF, DOC, DOCX, JPG, JPEG, PNG maksimal 10MB</p>
                            @error('file_surat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="5"
                        ...
                        placeholder="Masukkan keterangan">{{ old('keterangan', $suratKeluar->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
