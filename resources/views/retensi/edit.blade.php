...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Form Edit Data Retensi</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('retensi.update', $retensi->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4"
                    ...
                    placeholder="Masukkan keterangan retensi">{{ old('keterangan', $retensi->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
