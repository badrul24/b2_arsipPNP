                    ...
                    @foreach ($suratMasuks as $suratMasuk)
                        <tr class="hover:bg-gray-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                ...
                                @if(($currentUser->isOperator() && $currentUser->id === $suratMasuk->user_id && in_array($suratMasuk->status_surat, ['Diajukan', 'Ditolak'])) || $currentUser->isAdmin())
                                ...
                                @if($currentUser->isSekretaris())
                                    @if(in_array($suratMasuk->status_surat, ['Diajukan', 'Ditolak'])) {{-- Hanya jika 'Diajukan' atau 'Ditolak' --}}
                                ...
                                @if($currentUser->isPimpinan() && $suratMasuk->status_surat === 'Diproses') {{-- Jika statusnya 'Diproses', artinya menunggu Pimpinan untuk disposisi --}}
                                    <a href="{{ route('disposisi.create', ['surat_masuk_id' => $suratMasuk->id]) }}" title="Proses Disposisi"
                                ...
                                @if($currentUser->id && $suratMasuk->status_surat === 'Didisposisi')
                                    @php
                                        ...
                                        $disposisiTujuan = \App\Models\Disposisi::where('surat_masuk_id', $suratMasuk->id)
                                    ...
                                    @endphp
                                    @if($disposisiTujuan)
                                        <a href="{{ route('disposisi.show', $disposisiTujuan->id) }}" title="Lihat Disposisi"
                                        ...
                                        @if($disposisiTujuan->status_disposisi !== 'Selesai')
                                            <button type="button" title="Selesaikan Disposisi"
                                                ...
                                                data-disposisi-id="{{ $disposisiTujuan->id }}" data-surat-masuk-id="{{ $suratMasuk->id }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.verifikasi-btn, .kembalikan-btn, .teruskan-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (action === 'verifikasi') {
                    ...
                    text = 'Surat akan dikembalikan ke pengagenda untuk revisi. Mohon berikan alasan.';
                    icon = 'warning';
                    input = 'textarea';
                    ...
                    text = 'Surat akan diteruskan ke Pimpinan untuk disposisi. Pilih pimpinan tujuan:';
                    icon = 'question';
                    input = 'select';
                    ...
                    if (pimpinanData && pimpinanData.length > 0) {
                        ...
                        Swal.fire('Informasi', 'Tidak ada pimpinan ditemukan untuk diteruskan.', 'info');
                        return;
                    }
                    showSwalPrompt();
                }
                ...
                function showSwalPrompt() {
                    Swal.fire({
                        ...
                        icon: icon,
                        showCancelButton: true,
                        confirmButtonText: (action === 'kembalikan' ? 'Kembalikan' : (action === 'verifikasi' ? 'Verifikasi' : 'Teruskan')),
                        cancelButtonText: 'Batal',
                        reverseButtons: true,
                        input: input,
                        ...
                        if (result.isConfirmed) {
                            ...
                            fetch(`{{ route('surat_masuk.proses', ['surat_masuk' => ':id']) }}`.replace(':id', suratMasukId), {
                                method: 'POST',
                                body: formData
                            })
                            .then(response => {
        ...
        document.querySelectorAll('.status-detail-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                let fileHtml = '';
                if (filePath) {
                    fileHtml = `<p><strong>File:</strong> <a href="${filePath}" target="_blank" class="text-blue-600 hover:underline">${fileName}</a></p>`;
                } else {
            ...
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        formData.append('surat_masuk_id', suratMasukId); // Kirim surat_masuk_id juga
