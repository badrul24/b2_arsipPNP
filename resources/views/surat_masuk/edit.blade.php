            ...
            <form action="{{ route('surat_masuk.update', $suratMasuk->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_surat_pengirim" id="tanggal_surat_pengirim"
                                ...
                                value="{{ old('tanggal_surat_pengirim', optional($suratMasuk->tanggal_surat_pengirim)->format('Y-m-d')) }}">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_terima" id="tanggal_terima"
                                ...
                                value="{{ old('tanggal_terima', optional($suratMasuk->tanggal_terima)->format('Y-m-d')) }}">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label for="status_surat" class="block font-medium text-gray-700 mb-1">Status Surat</label>
                            <select name="status_surat" id="status_surat"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('status_surat') border-red-500 @enderror"
                                required>
                                <option value="" disabled selected>Pilih Status</option>
                                <option value="Baru" {{ old('status_surat', $suratMasuk->status_surat) == 'Baru' ? 'selected' : '' }}>Baru</option>
                                <option value="Dibaca" {{ old('status_surat', $suratMasuk->status_surat) == 'Dibaca' ? 'selected' : '' }}>Dibaca</option>
                                <option value="Dikonfirmasi" {{ old('status_surat', $suratMasuk->status_surat) == 'Dikonfirmasi' ? 'selected' : '' }}>Dikonfirmasi</option>
                                <option value="Didisposisi" {{ old('status_surat', $suratMasuk->status_surat) == 'Didisposisi' ? 'selected' : '' }}>Didisposisi</option>
                                <option value="Selesai" {{ old('status_surat', $suratMasuk->status_surat) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                <option value="Diarsipkan" {{ old('status_surat', $suratMasuk->status_surat) == 'Diarsipkan' ? 'selected' : '' }}>Diarsipkan</option>
                            </select>
                            @error('status_surat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            @if($suratMasuk->file_surat_path)
                                <div class="mb-2">
                                    ...
                                    <a href="{{ asset($suratMasuk->file_surat_path) }}" target="_blank" class="text-primary-600 hover:text-primary-500">
                                        Lihat File
                                    </a>
                                </div>
                            @endif
