...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Tambah Status Dokumen</h2>
            <p class="mt-1 text-sm text-gray-500">Form Tambah Data Status Dokumen</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="max-w-xl">
            <form action="{{ route('statusDokumen.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                        ...
                        placeholder="Masukkan deskripsi status">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
