...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Jenis Dokumen</h2>
            <p class="mt-1 text-sm text-gray-500">Form Edit Data Jenis Dokumen</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="max-w-xl">
            <form action="{{ route('jenis-dokumen.update', $jenisDokumen->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block font-medium text-gray-700 mb-1">Kode Jenis</label>
                    <input type="text" name="kode_jenis"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('kode_jenis') border-red-500 @enderror"
                        value="{{ old('kode_jenis', $jenisDokumen->kode_jenis) }}"
                        placeholder="Masukkan kode jenis (contoh: SK, ND, SP)">
                    @error('kode_jenis')
                ...
                <div>
                    <label class="block font-medium text-gray-700 mb-1">Nama Jenis</label>
                    <input type="text" name="nama_jenis"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('nama_jenis') border-red-500 @enderror"
                        value="{{ old('nama_jenis', $jenisDokumen->nama_jenis) }}"
                        placeholder="Masukkan nama jenis (contoh: Surat Keputusan, Nota Dinas, Surat Pengantar)">
                    @error('nama_jenis')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                        ...
                        placeholder="Masukkan keterangan jenis dokumen">{{ old('keterangan', $jenisDokumen->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
