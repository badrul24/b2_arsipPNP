...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Tambah Divisi</h2>
            <p class="mt-1 text-sm text-gray-500">Tambah data divisi baru</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('divisi.store') }}" method="POST" class="space-y-6">
            @csrf

            <div>
                <label for="kode_divisi" class="block text-sm font-medium text-gray-700 mb-1">Kode Divisi</label>
                <input type="text" name="kode_divisi" id="kode_divisi" value="{{ old('kode_divisi') }}"
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4"
                    ...
                    placeholder="Masukkan keterangan divisi">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
